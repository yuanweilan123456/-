...
<template>
  <div class="page-wrap file-tool-page">
    <RouterLink class="back-link" to="/toolbox">← {{ isZh ? '返回工具箱' : 'Back to toolbox' }}</RouterLink>
    <p class="eyebrow"><span class="eyebrow-dot"></span>PIXELFORGE FILE TOOLBOX</p>
    <h1>{{ configs.title }}</h1>
    <p class="file-tool-description">{{ configs.description }}</p>
    <section class="file-tool-panel">
      <input ref="input" class="visually-hidden" type="file" :accept="configs.accept" :multiple="configs.multiple" @change="onChange" />
      ...
      <div v-if="files.length" class="file-tool-actions"><button class="process-button" type="button" :disabled="isProcessing" @click="run">{{ isProcessing ? (isZh ? '处理中…' : 'Processing…') : configs.action }}<span aria-hidden="true">→</span></button><button class="text-button" type="button" :disabled="isProcessing" @click="clear">{{ isZh ? '清空' : 'Clear' }}</button></div>
    ...
    <div class="ad-slot" aria-label="Advertisement placeholder"><span>{{ isZh ? '广告位预留 · 上线后启用' : 'Ad space reserved · Enabled after launch' }}</span></div>
  </div>
</template>
